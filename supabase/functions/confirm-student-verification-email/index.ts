...
serve(async (req) => {
  ...
  try {
    ...
    const { data: rec, error: recErr } = await supabase
      ...
      .select("user_id, university_email, code_hash, code_expires_at, attempts, verified_at")
      .eq("user_id", user.id)
      .maybeSingle();
    ...
    const codeSecret = Deno.env.get("STUDENT_EMAIL_CODE_SECRET");
    if (!codeSecret) {
      return jsonResponse({
        success: false,
    ...
    const { error: verifyErr } = await supabase
      ...
      .update({
        university_email: rec.university_email,
    ...
    await supabase.from("notifications").insert({
      ...
      message: "Your university email has been verified. Your Verified Student badge is now active.",
      link: "/app/profile/me",
    });

    return jsonResponse({ success: true, message: "Email verified successfully" });
  } catch (err: any) {
    console.error("[confirm-student-verification-email]", err);
