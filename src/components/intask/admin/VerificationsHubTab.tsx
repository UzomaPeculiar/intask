...
function StudentVerificationsTab() {
  ...
  const { data: pending, isLoading, refetch } = useQuery({ queryKey: ["pending-students"], queryFn: async () => { const { data, error } = await supabase.from("admin_student_profiles").select("*, profile:admin_profiles!student_profiles_user_id_fkey(id, full_name, email)").eq("verified", false).eq("verification_method", "id_upload").order("created_at", { ascending: true }); if (error) throw error; return data ?? []; } });
...
function IndividualVerificationsTab() {
  ...
  const [viewingImage, setViewingImage] = useState<string | null>(null);
  const [viewingProfile, setViewingProfile] = useState<string | null>(null);
  const { data: pending, isLoading } = useQuery({ queryKey: ["pending-individuals"], queryFn: async () => { const { data, error } = await (supabase as any).from("admin_individual_profiles").select("*, profile:admin_profiles!individual_profiles_user_id_fkey(id, full_name, email)").eq("verification_status", "pending_review").order("created_at", { ascending: true }); if (error) throw error; return data ?? []; } });
