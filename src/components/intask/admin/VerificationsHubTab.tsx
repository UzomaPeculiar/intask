  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const [studentsRes, companiesRes, individualsRes, profilesRes] = await Promise.all([
        (supabase as any).from("student_profiles").select("user_id, verified, verification_status, verification_method, created_at, updated_at"),
        (supabase as any).from("company_profiles").select("user_id, verified, verification_status, created_at, verified_at, updated_at"),
        (supabase as any).from("individual_profiles").select("user_id, verified, verification_status, created_at, verified_at, updated_at"),
...
function StudentVerificationsTab() {
  ...
  const { data: pending, isLoading, refetch } = useQuery({ queryKey: ["pending-students"], queryFn: async () => { const { data, error } = await supabase.from("student_profiles").select("*, profile:profiles!student_profiles_user_id_fkey(id, full_name, email)").eq("verified", false).eq("verification_method", "id_upload").order("created_at", { ascending: true }); if (error) throw error; return data ?? []; } });
...
function IndividualVerificationsTab() {
  ...
  const [viewingImage, setViewingImage] = useState<string | null>(null);
  const [viewingProfile, setViewingProfile] = useState<string | null>(null);
  const { data: pending, isLoading } = useQuery({ queryKey: ["pending-individuals"], queryFn: async () => { const { data, error } = await (supabase as any).from("individual_profiles").select("*, profile:profiles!individual_profiles_user_id_fkey(id, full_name, email)").eq("verification_status", "pending_review").order("created_at", { ascending: true }); if (error) throw error; return data ?? []; } });
...
function CompanyVerificationsTab() {
  ...
  const [viewingImage, setViewingImage] = useState<string | null>(null);
  const [viewingProfile, setViewingProfile] = useState<string | null>(null);
  const { data: pending, isLoading, refetch } = useQuery({ queryKey: ["pending-companies"], queryFn: async () => { const { data, error } = await supabase.from("company_profiles").select("*, profile:profiles!company_profiles_user_id_fkey(id, full_name, email)").eq("verified", false).order("created_at", { ascending: true }); if (error) throw error; return data ?? []; } });
