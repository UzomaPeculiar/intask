...
import { adminApproveStudentVerification, adminRejectStudentVerification } from "@/lib/admin.functions";
...
function StudentVerificationsTab() {
  ...
  const approve = useMutation({ mutationFn: async (userId: string) => { await adminApproveStudentVerification({ data: { userId } }); }, onSuccess: () => { toast.success("Student verified successfully"); refetch(); qc.invalidateQueries({ queryKey: ["admin-command-center"] }); qc.invalidateQueries({ queryKey: ["admin-verifications-hub"] }); qc.invalidateQueries({ queryKey: ["admin-users-management"] }); }, onError: (e: any) => toast.error(e.message ?? "Could not approve") });
  const reject = useMutation({ mutationFn: async (userId: string) => { await adminRejectStudentVerification({ data: { userId } }); }, onSuccess: () => { toast.success("Student rejected and notified"); refetch(); qc.invalidateQueries({ queryKey: ["admin-command-center"] }); qc.invalidateQueries({ queryKey: ["admin-verifications-hub"] }); qc.invalidateQueries({ queryKey: ["admin-users-management"] }); }, onError: (e: any) => toast.error(e.message ?? "Could not reject") });
