    ...
    <div className="min-h-screen bg-[#F8FAFC] text-[#1E293B] [font-family:'Inter',sans-serif]">
      <div className="mx-auto w-full max-w-[640px] px-6 pb-40 pt-7 sm:px-12">
        <button
          ...
          aria-label="Back"
          className="mb-4 inline-flex size-9 items-center justify-center rounded-full border border-[#E2E8F0] bg-white"
        >
          <ArrowLeft className="size-4 text-[#1E293B]" />
        </button>

        <h1 className="font-['Space_Grotesk',sans-serif] text-[1.5rem] font-bold text-[#1E293B]">Fund escrow</h1>
        <p className="mt-1 text-[0.85rem] leading-[1.5] text-[#6B7280]">Pay safely to lock in this student. Money is only released after you approve the work.</p>

        <div className="mt-6 rounded-[18px] border border-[#E2E8F0] bg-white p-7 shadow-[0_18px_50px_-24px_rgba(37,99,235,0.32)]">
          <p className="text-[0.7rem] font-semibold uppercase tracking-[0.1em] text-[#94A3B8]">Task amount</p>
          <p className="mt-1.5 font-['Space_Grotesk',sans-serif] text-[2rem] font-bold text-[#1E293B]">{naira(total)}</p>

          <div className="my-5 h-px bg-[#E2E8F0]" />

          <p className="text-[0.7rem] font-semibold uppercase tracking-[0.1em] text-[#94A3B8]">Wallet</p>
          <p className="mt-1 text-[0.85rem] text-[#1E293B]">
          ...
          <div className="mt-5 space-y-2">
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-[12px] border p-[14px] text-left transition-all ${paymentMethod === "wallet" ? "border-[#16A34A] bg-[rgba(22,163,74,0.04)]" : "border-[#E2E8F0] hover:border-[#E2E8F0] hover:bg-[#FFFFFF]"}`}
            >
              <span className={`flex size-[18px] items-center justify-center rounded-full border-2 ${paymentMethod === "wallet" ? "border-[#16A34A]" : "border-[#E2E8F0]"}`}>
                {paymentMethod === "wallet" ? <span className="size-2 rounded-full bg-[#16A34A]" /> : null}
              </span>
              <span className="flex-1">
                <span className="block text-[0.85rem] font-semibold text-[#1E293B]">Use Wallet</span>
                <span className="mt-0.5 block text-[0.7rem] text-[#6B7280]">
                  {shortfall <= 0 ? "Pay instantly from your wallet balance" : `Use ${naira(walletContribution)} from wallet`}
                </span>
              </span>
              <Wallet className="size-5 text-[#15803D]" />
            </button>

            <button
              type="button"
              onClick={() => setPaymentMethod("paystack")}
              className={`flex w-full items-center gap-3 rounded-[12px] border p-[14px] text-left transition-all ${paymentMethod === "paystack" ? "border-[#16A34A] bg-[rgba(22,163,74,0.04)]" : "border-[#E2E8F0] hover:border-[#E2E8F0] hover:bg-[#FFFFFF]"}`}
            >
              <span className={`flex size-[18px] items-center justify-center rounded-full border-2 ${paymentMethod === "paystack" ? "border-[#16A34A]" : "border-[#E2E8F0]"}`}>
                {paymentMethod === "paystack" ? <span className="size-2 rounded-full bg-[#16A34A]" /> : null}
              </span>
              <span className="flex-1">
                <span className="block text-[0.85rem] font-semibold text-[#1E293B]">Pay with Card/Bank</span>
                <span className="mt-0.5 block text-[0.7rem] text-[#6B7280]">Powered by Paystack — secure card or bank transfer</span>
              </span>
              <CreditCard className="size-5 text-[#15803D]" />
            </button>
          </div>

          <div className="mt-5 flex items-start gap-2 rounded-[10px] border border-[#E2E8F0] bg-[#F1F3F5] p-[14px] text-[0.75rem] leading-[1.5] text-[#6B7280]">
      ...
      <div className="fixed inset-x-0 bottom-16 z-20 border-t border-[#E2E8F0] bg-white px-6 py-4 lg:bottom-0 lg:px-12">
        <div className="mx-auto w-full max-w-[640px]">
          ...
          {paymentMethod === "wallet" && shortfall > 0 ? (
            <Button
              variant="ghost"
              className="mt-2 h-9 w-full text-[0.78rem] font-medium text-[#15803D] hover:bg-[#F1F3F5]"
